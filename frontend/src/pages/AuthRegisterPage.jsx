...
const AuthRegisterPage = () => {
  ...
  const validateForm = () => {
    ...
    if (phone && !validatePhone(phone)) {
      newErrors.phone = 'Invalid phone format (use E.164: +33...)';
    }
    if (!acceptTerms) {
  ...
  return (
    <div className="min-h-screen bg-white dark:bg-black pt-24 pb-24">
      <div className="max-w-xl md:max-w-2xl mx-auto px-6">
        <h1 className="text-4xl md:text-5xl font-bold mb-8 text-gray-900 dark:text-gray-100">Sign up</h1>
        {error && <div className="mb-4 p-3 text-sm text-red-600 bg-red-50 dark:bg-red-900/20 rounded-lg">{error}</div>}

        {/* OAuth Section - Priority */}
        <div className="apple-card p-8 md:p-10 mb-6">
          <h2 className="text-xl font-semibold mb-4 text-center">Quick sign up</h2>
          <button 
            type="button" 
        ...
        <form onSubmit={onSubmit} className="space-y-6 apple-card p-8 md:p-10 mb-6">
          <h2 className="text-xl font-semibold">Create account</h2>
          
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <label className="block text-sm md:text-base mb-2">First name *</label>
              <input 
                className={`w-full border rounded-lg px-4 py-3 text-base ${errors.firstName ? 'border-red-500' : ''}`}
                value={firstName} 
                onChange={(e) => {
            ...
            <div>
              <label className="block text-sm md:text-base mb-2">Last name *</label>
              <input 
                className={`w-full border rounded-lg px-4 py-3 text-base ${errors.lastName ? 'border-red-500' : ''}`}
                value={lastName} 
                onChange={(e) => {
          ...
          <div>
            ...
            <input 
              className={`w-full border rounded-lg px-4 py-3 text-base ${errors.email ? 'border-red-500' : ''}`}
              type="email" 
              value={email} 
              onChange={(e) => {
          ...
          <div>
            <label className="block text-sm md:text-base mb-2">Phone (optional)</label>
            <input 
              className={`w-full border rounded-lg px-4 py-3 text-base ${errors.phone ? 'border-red-500' : ''}`}
              type="tel" 
          ...
          <div>
            ...
            <input 
              className={`w-full border rounded-lg px-4 py-3 text-base ${errors.password ? 'border-red-500' : ''}`}
              type="password" 
              value={password} 
              onChange={(e) => {
          ...
          <div>
            ...
            <input 
              className={`w-full border rounded-lg px-4 py-3 text-base ${errors.confirmPassword ? 'border-red-500' : ''}`}
              type="password" 
          ...
          <div className="space-y-3">
            ...
            {errors.acceptTerms && <p className="text-xs text-red-600 ml-7">{errors.acceptTerms}</p>}
